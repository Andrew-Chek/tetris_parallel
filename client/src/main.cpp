...
int main() {
    ...
    bool game1Over = false, game2Over = false;
    ...
    std::thread game1Thread(runGameLogic, std::ref(game1), std::ref(game1Running), std::ref(game1Over));
    std::thread game2Thread(runGameLogic, std::ref(game2), std::ref(game2Running), std::ref(game2Over));
                ...
                switch (event.key.keysym.sym) {
                    ...
                    case SDLK_r:
                        if (game1Over && game2Over) {
                            ...
                            // Restart threads for both games
                            game1Running = true;
                            game2Running = true;
                            game1Thread = std::thread(runGameLogic, std::ref(game1), std::ref(game1Running), std::ref(game1Over));
                            game2Thread = std::thread(runGameLogic, std::ref(game2), std::ref(game2Running), std::ref(game2Over));
                        }
                        break;
